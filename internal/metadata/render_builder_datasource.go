package metadata

import (
	"bytes"
	"fmt"
	"io"
	"maps"
	"slices"

	"github.com/hashicorp/hcl/v2/hclwrite"
)

type datasourceRenderBuilder struct {
	ProviderName   string
	DataSourceType string

	Metadata DataSourceMetadata

	Subcategory string
	Examples    []Example
}

func (b datasourceRenderBuilder) renderHeader(w io.Writer) error {
	if _, err := fmt.Fprintf(w, `---
# generated by https://github.com/magodo/tfproviderdocs
page_title: "%[1]s Data Source - %[2]s"
subcategory: "%[3]s"
description: |-
  %[4]s
---
`, b.DataSourceType, b.ProviderName, b.Subcategory, b.Metadata.Schema.Description); err != nil {
		return err
	}
	return nil
}

func (b datasourceRenderBuilder) renderDescription(w io.Writer) error {
	if _, err := fmt.Fprintf(w, `# %s (Data Source)
`, b.DataSourceType); err != nil {
		return err
	}

	if deprecation := b.Metadata.Schema.Deprecation; deprecation != "" {
		if _, err := fmt.Fprintf(w, `
!> %s
`, deprecation); err != nil {
			return nil
		}
	}

	if _, err := fmt.Fprintf(w, `
%s
`, b.Metadata.Schema.Description); err != nil {
		return err
	}

	return nil
}

func (b datasourceRenderBuilder) renderExample(w io.Writer) error {
	if examples := b.Examples; len(examples) != 0 {
		if _, err := io.WriteString(w, "## Example Usage\n"); err != nil {
			return err
		}
		for _, example := range examples {
			if example.Header != nil {
				if _, err := fmt.Fprintf(w, "\n### %s\n", *example.Header); err != nil {
					return err
				}
			}
			if example.Description != nil {
				if _, err := fmt.Fprintf(w, "\n%s\n", *example.Description); err != nil {
					return err
				}
			}
			if example.HCL != nil {
				if _, err := fmt.Fprintf(w, "\n```terraform\n%s\n```\n", bytes.TrimSpace(hclwrite.Format(example.HCL))); err != nil {
					return err
				}
			}
		}
	}
	return nil
}

func (b datasourceRenderBuilder) renderSchema(w io.Writer) error {
	if _, err := fmt.Fprintf(w, `## Schema
`); err != nil {
		return err
	}

	sections := []struct {
		name   string
		fields []DataSourceField
	}{
		{
			name:   "Required",
			fields: b.Metadata.Schema.Fields.RequiredFields(),
		},
		{
			name:   "Optional",
			fields: b.Metadata.Schema.Fields.OptionalFields(),
		},
		{
			name:   "Read-Only",
			fields: b.Metadata.Schema.Fields.ComputedFields(),
		},
	}

	for _, section := range sections {
		if len(section.fields) == 0 {
			continue
		}
		if _, err := fmt.Fprintf(w, `
### %s
	
`, section.name); err != nil {
			return err
		}

		for _, field := range section.fields {
			if err := b.renderField(w, field); err != nil {
				return err
			}
		}
	}

	if nested := b.Metadata.Schema.Nested; len(nested) != 0 {
		io.WriteString(w, "\n")
		if err := b.renderNestedFields(w, nested); err != nil {
			return err
		}
	}

	return nil
}

func (b datasourceRenderBuilder) renderNestedFields(w io.Writer, fields DataSourceNestedFields) error {
	keys := slices.Collect(maps.Keys(fields))
	slices.Sort(keys)
	for _, key := range keys {
		field := fields[key]

		if _, err := fmt.Fprintf(w, `<a id="nested--%[1]s"></a>
### Nested Schema for %[2]s%[1]s%[2]s
`, key, "`"); err != nil {
			return err
		}

		if l := field.Validators; len(l) != 0 {
			io.WriteString(w, "\nValidators:\n\n")
			for _, e := range l {
				if _, err := fmt.Fprintf(w, "- %s\n", e); err != nil {
					return err
				}
			}
		}

		sections := []struct {
			name   string
			fields []DataSourceField
		}{
			{
				name:   "Required",
				fields: field.Fields.RequiredFields(),
			},
			{
				name:   "Optional",
				fields: field.Fields.OptionalFields(),
			},
			{
				name:   "Read-Only",
				fields: field.Fields.ComputedFields(),
			},
		}

		for _, section := range sections {
			if len(section.fields) == 0 {
				continue
			}
			if _, err := fmt.Fprintf(w, `
%s:

`, section.name); err != nil {
				return err
			}

			for _, field := range section.fields {
				if err := b.renderField(w, field); err != nil {
					return err
				}
			}
		}
	}

	return nil
}

func (b datasourceRenderBuilder) renderField(w io.Writer, field DataSourceField) error {
	if _, err := fmt.Fprintf(w, "- `%s` (%s) %s", field.Name, field.Traits(), field.Description); err != nil {
		return err
	}
	if field.NestedLink() != "" {
		fmt.Fprintf(w, " %s", field.NestedLink())
	}
	io.WriteString(w, "\n")

	if l := field.Validators(); len(l) != 0 {
		io.WriteString(w, "\n\tValidators:\n\n")
		for _, e := range l {
			if _, err := fmt.Fprintf(w, "\t- %s\n", e); err != nil {
				return err
			}
		}
	}

	if field.Deprecation != "" {
		if _, err := fmt.Fprintf(w, "\n\t!> %s\n", field.Deprecation); err != nil {
			return err
		}
	}

	return nil
}
